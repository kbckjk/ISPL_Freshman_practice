...
tf.flags.DEFINE_string('test_datadir', 'mnist/test_tfrecord', 'directory where test tfrecord files are stored in')
...
tf.flags.DEFINE_integer('epoch', 10, 'total number of epochs for training')
...
def main(args):
    ...
    if FLAGS.process == 'write':
        write_tfrecord(FLAGS.imagedir, FLAGS.datadir+'/mnist.tfrecord', FLAGS.val_datadir+'/mnist.tfrecord', 0.8)
    else:
